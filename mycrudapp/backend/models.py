...
class UserField(models.Model):
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name='user_field')
...
class PostField(models.Model):
    ...
    user = models.ForeignKey(
        UserField, on_delete=models.CASCADE, related_name='posts')
    post_title = models.CharField(max_length=500, blank=False, null=False)
    description = models.TextField(blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    # you can do (primary_key=True) if you dont want to use "id" # instead u post_id see 191 on views.py
...
class ReplyField(models.Model):
    post = models.ForeignKey(
        PostField, on_delete=models.CASCADE, related_name='replies')
